...
const message = computed(() => extractValue(config.value.message?.html || config.value.message) || '')
const acceptText = computed(() => extractValue(config.value.accept?.html || config.value.accept_text) || 'OK')
const declineText = computed(() => extractValue(config.value.decline?.html || config.value.decline_text) || '')
const showDecline = computed(() => config.value.show_decline === true)
const policyUrl = computed(() => config.value.policy_url || '')
const policyLinkText = computed(() => extractValue(config.value.policy?.html || config.value.policy_link_text) || '')
